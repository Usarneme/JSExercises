// Given two buckets of different size, demonstrate how to measure an exact number of liters.
  // moves() = int # of moves required to end up at the desired number of liters
  // goalBucket = str which bucket ('one' or 'two') will contain the liquid
  // otherBucket = int how many liters of liquid are in the other bucket when finished
TwoBucket = function(size1, size2, goal, start) {
  var bucket1 = new Bucket(size1, 'one');
  var bucket2 = new Bucket(size2, 'two');
  this.goal = goal;
  this.moveCount = 1;
  this.goalBucket;
  this.otherBucket;
...
TwoBucket.prototype.start = function(bucket1, bucket2) {
  while (true) {
    ...
    else if (bucket1.isEmpty()) {
      bucket1.fill();
    } else if (bucket1.isFull() && bucket2.isPartFull()) {
      bucket1.pour(bucket2);
    } else if (bucket2.isFull()) {
      bucket2.empty();
    } else if (bucket2.isEmpty() && bucket1.isPartFull()) {
      bucket1.pour(bucket2);
    }
    this.moveCount++;
  }
...
Bucket.prototype.isFull = function() {
  return this.contains == this.size;
};

Bucket.prototype.isPartFull = function() {
...
Bucket.prototype.pour = function(target) {
  var amountToTransfer = Math.min(target.size - target.contains, this.contains);
